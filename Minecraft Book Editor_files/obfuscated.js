function animate_obfuscated_text(text)
{
  ...
  for (var i = 0; i < text.length; i++)
  {
    ...
    if (len >= 0)
    {
      ...
      new_text += new_char; // si oon ne trouve pas de subsitue
  }
  return new_text;
}
...
function animate_obfuscated_tree(node)
{
  ...
  {
    var subnode = split[i];
    if (subnode.nodeType !== 3) // texte simple
      animate_obfuscated_tree(subnode); // recursion sur tous les noeuds
    else
...
{
  var ckframes = $('.cke_wysiwyg_frame').contents();

  var obfuscated_texts = $('.preview_zone').find('.obfuscated');
